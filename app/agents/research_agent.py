...
def run_research_agent(state: ResearchAssistantState) -> dict:
    ...
    if llm is not None:
        try:
            ...
            if classification:
                logger.error(f"Research Agent query formulation: unrecoverable API error: {classification}")
                return {
    ...
    # Step 2: Execute search and deduplicate by URL (ADR-003)
    ...
    if llm is not None:
        try:
            ...
            classification = classify_llm_error(e)
            if classification:
                logger.error(f"Research Agent self-evaluation: unrecoverable API error: {classification}")
                return {
                    "degraded_mode": classification,
